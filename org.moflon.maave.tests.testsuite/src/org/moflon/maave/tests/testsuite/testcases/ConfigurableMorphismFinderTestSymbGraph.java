...
public class ConfigurableMorphismFinderTestSymbGraph {
   ...
   public void test() {
      ...
      for (int i=1; i<=7 ;i++) {
         ...
         MoflonEOperation operationAdd=(MoflonEOperation) cls.getEOperations().stream().filter(x->x.getName().equals("testPatternAdd")).findFirst().get();
         MoflonEOperation operationLarger4=(MoflonEOperation) cls.getEOperations().stream().filter(x->x.getName().equals("testPatternLarger4")).findFirst().get();
         Assert.assertTrue("FailedAssert: 0",operationAdd!=null);
         Assert.assertTrue("FailedAssert: 1",operationLarger4!=null);
         
         
         StoryNode stnAdd=(StoryNode) operationAdd.getActivity().getOwnedActivityNode().stream().filter(x->x instanceof StoryNode).collect(Collectors.toList()).get(0);
         StoryNode stnLarger4=(StoryNode) operationLarger4.getActivity().getOwnedActivityNode().stream().filter(x->x instanceof StoryNode).collect(Collectors.toList()).get(0);
         Transformer transformer=StptransformationFactory.eINSTANCE.createTransformer();
         SymbGTRule ruleAdd=transformer.transformStpToSymbGTRule(stnAdd.getStoryPattern());
         SymbGTRule ruleLarger4=transformer.transformStpToSymbGTRule(stnLarger4.getStoryPattern());
         
         
         SymbolicGraph L=ruleAdd.getLeft().getCodom();
         SymbolicGraph G=ruleLarger4.getLeft().getDom();
...
//         
         ...
         ConfigurableMorphismFinder confMorFinder = mofFindFac.createMorphismFinder(L, morclass);
        
         SymbolicGraphMorphism emptyMatch =SymbolicGraphMorphismsFactory.eINSTANCE.createSymbolicGraphMorphism();
         emptyMatch.setDom(L);
         emptyMatch.setCodom(G);
         
         
         MorphismsSet matches=confMorFinder.getAllMorphisms(emptyMatch);
         int x=0;
         x++;
         x++;
//         
