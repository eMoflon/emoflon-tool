...
public class ILPSolverUtil {
	...
	public Result solve(Graph sourceGraph, Graph targetGraph, ConsistencyCheckPrecedenceGraph protocol) {

		createIlpProblemFromGraphs(sourceGraph, targetGraph, protocol);

		variableCount = ilpProblem.getVars().size();
		constraintCount = ilpProblem.getConstraints().size();

		try {
			// solve
			Result result = ilpProblem.solve(ILPSolverStrategy.getInstance().getSolver());
	...
	protected void createIlpProblemFromGraphs(Graph sourceGraph, Graph targetGraph,
