 ...
 * and determining what is "source" and "target".
 * 
 * @author anjorin
 *
 */
public class BackwardSynchronizer extends Synchronizer
{
   ...
   public BackwardSynchronizer(CorrespondenceModel graphTriple, Delta delta, SynchronizationProtocol protocol, Configurator configurator, StaticAnalysis rules, TempOutputContainer tempOutputContainer)
   {
      ...
      lookupMethods = rules.getTargetRules();
   }
   ...
   protected Graph delete(Collection<TripleMatch> allToBeRevokedTripleMatches)
   {
      ...
      return allToBeRevokedTripleMatches.stream().map(TripleMatch::getCreatedTrgElts).reduce(Graph.getEmptyGraph(), Graph::addConstructive);
   }
   ...
   protected void finalizeGraphTriple(CorrespondenceModel graphTriple)
   {
      ...
      if (tempOutputContainer.getPotentialRoots().size() == 1)
      {
         ...
         tempOutputContainer.eResource().getResourceSet().createResource(eMoflonEMFUtil.createFileURI("sourceModel", false)).getContents().add(source);
      } else
