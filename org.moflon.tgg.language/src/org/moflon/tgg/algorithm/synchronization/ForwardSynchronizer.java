...
/**
 * A specialization of {@link Synchronizer} for the forward direction.
 * This includes deleting, finalizing the results, and determining what is "source" and "target".
 * 
 * @author anjorin
 *
 */
public class ForwardSynchronizer extends Synchronizer
{
   ...
   public ForwardSynchronizer(CorrespondenceModel graphTriple, Delta delta, SynchronizationProtocol protocol, Configurator configurator, StaticAnalysis rules, TempOutputContainer tempOutputContainer)
   {
      ...
      lookupMethods = rules.getSourceRules();
   }
   ...
   protected Graph delete(Collection<TripleMatch> allToBeRevokedTripleMatches)
   {
      ...
      return allToBeRevokedTripleMatches.stream().map(TripleMatch::getCreatedSrcElts).reduce(Graph.getEmptyGraph(), Graph::addConstructive);
   }
   ...
   protected void finalizeGraphTriple(CorrespondenceModel graphTriple)
   {
      ...
      if (tempOutputContainer.getPotentialRoots().size() == 1)
      {
         ...
         tempOutputContainer.eResource().getResourceSet().createResource(eMoflonEMFUtil.createFileURI("targetModel", false)).getContents().add(target);
      } else
