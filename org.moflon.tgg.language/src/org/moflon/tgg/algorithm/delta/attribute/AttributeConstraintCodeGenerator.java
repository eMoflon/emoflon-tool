...
public class AttributeConstraintCodeGenerator {
	...
	private void handleCSPSolving() {
		Collection<Variable> allFreeVars = ocsp.getVariables().stream().filter(v -> !v.isBound())
				.collect(Collectors.toList());
		...
		for (TGGConstraint c : ocsp.getConstraints()) {
			...
			Collection<String> solveVars = c.getVariables().stream().map(v -> varToLabel.get(v))
					.collect(Collectors.toList());
			container.setSolveVars(solveVars);

			Collection<Variable> freeVars = allFreeVars.stream()
