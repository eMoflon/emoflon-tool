...
public class LimitedRandomRuleSelector extends RandomRuleSelector
{
   ...
   public EOperation getNextGenModelOperation(List<EOperation> rulesToBeApplied, ModelgenStats modelgenStats)
   {
      ...
      while (true)
      {
         ...
         if (modelgenStats.getRulePerformStats().containsKey(ruleName) && ruleNameToPerformCount.containsKey(ruleName))
         {
            // current limit is lower than defined limit
            if (modelgenStats.getRulePerformStats().get(ruleName).getPerformCount() < ruleNameToPerformCount.get(ruleName))
            {
