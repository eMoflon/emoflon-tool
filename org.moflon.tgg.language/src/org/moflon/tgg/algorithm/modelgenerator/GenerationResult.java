...
public class GenerationResult
{
   ...
   public void logResult()
   {
      ...
      logger.info("performs: " + modelgenStats.getTotalRulePerformCount() + " / duration " + modelgenStats.getTotalDuration() + "ms");
      logger.info("failures: " + modelgenStats.getTotalFailureCount() + " / failure duration: " + modelgenStats.getTotalFailureDuration() +"ms");
      logger.info(modelgenStats.getSrcElementCount() + "/" + modelgenStats.getSrcConnectorCount() + " nodes/edges created for source");
      logger.info(modelgenStats.getTrgElementCount() + "/" + modelgenStats.getTrgConnectorCount() + " nodes/edges created for target");
      logger.info(modelgenStats.getCorrElementCount() + "/" + modelgenStats.getCorrConnectorCount() + " nodes/edges created for correspondence");
      logger.info("");
      for (RulePerformStats stats : modelgenStats.getRulePerformStats().values())
      {
         ...
         logger.info("   failures: " + stats.getFailures() + " / failure duration: " + stats.getTotalFailureDuration() + "ms");
      }
      logger.info("--- Model Generation Log ---");
