...
public class AddPrefix extends TGGConstraintImpl
{
   ...
   /**
    * addPrefix(prefix,word,result) prefix+word = result (prefix concatenated with word equals result)
    * 
   ...
   public void solve(Variable prefix, Variable word, Variable result)
   {
      ...
      else if (bindingStates.equals("FBB"))
      {
         ...
         {
            prefix.setValue(((String) result.getValue()).substring(0, ((String) result.getValue()).length() - ((String) word.getValue()).length()));
