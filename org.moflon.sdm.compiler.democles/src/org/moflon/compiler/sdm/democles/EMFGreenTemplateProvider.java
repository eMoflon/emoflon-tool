		...
		if (type instanceof StructuralFeature<?>) {
			...
			if (adornment.get(0) == Adornment.BOUND && adornment.get(1) == Adornment.BOUND) {
				if (sf.getLinkedElement().isMany()) {
					return new Chain<TemplateController>(new TemplateController("/emf-create/CreateToManyLink", operation), tail);
				} else if (sf.getLinkedElement().isChangeable()) {
					return new Chain<TemplateController>(new TemplateController("/emf-create/CreateToOneLink", operation), tail);
					...
					if (opposite.isMany()) {
						return new Chain<TemplateController>(new TemplateController("/emf-create/ReverseCreateToManyEReference", operation), tail);
					} else if (opposite.isChangeable()) {
						return new Chain<TemplateController>(new TemplateController("/emf-create/ReverseCreateToOneEReference", operation), tail);
					}
				}
			}
		} else if (type instanceof EMFVariable) {
			if (adornment.get(0) == Adornment.FREE) {
				return new Chain<TemplateController>(new TemplateController("/emf-create/CreateObject", operation), tail);
			}
		}
