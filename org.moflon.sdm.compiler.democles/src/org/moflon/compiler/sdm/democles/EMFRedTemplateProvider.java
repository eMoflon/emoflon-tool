		...
		if (type instanceof Reference) {
			...
			if (adornment.get(0) == Adornment.BOUND && adornment.get(1) == Adornment.BOUND) {
				if (ref.getLinkedElement().isMany()) {
					return new Chain<TemplateController>(new TemplateController("/emf-delete/DeleteToManyLink", operation), tail);
				} else if (ref.getLinkedElement().isChangeable()) {
					return new Chain<TemplateController>(new TemplateController("/emf-delete/DeleteToOneLink", operation), tail);
				} else if (ref.getLinkedElement().getEOpposite() != null) {
					final EReference opposite = ref.getLinkedElement().getEOpposite();
					if (opposite.isMany()) {
						return new Chain<TemplateController>(new TemplateController("/emf-delete/ReverseDeleteToManyEReference", operation), tail);
					} else if (opposite.isChangeable()) {
						return new Chain<TemplateController>(new TemplateController("/emf-delete/ReverseDeleteToOneEReference", operation), tail);
					}
				}
			}
		}
