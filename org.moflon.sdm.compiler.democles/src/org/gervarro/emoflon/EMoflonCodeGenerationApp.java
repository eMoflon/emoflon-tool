...
public class EMoflonCodeGenerationApp extends GenModelBuilder
{
   ...
   public GenModel buildGenModel(final URI genModelURI, final boolean saveGenModel) throws IOException
   {
      ...
      final URI ecoreJarURI = URI.createURI("file:/C:/Download/eclipse-juno/plugins/org.eclipse.emf.ecore_2.8.3.v20130125-0546.jar", true);
      resourceSet.getURIConverter().getURIMap().put(ecorePlatformPluginURI, GenModelBuilder.createArchiveURI(ecoreJarURI));
      ...
      final URI sdmLanguageJarURI = URI.createURI("file:/C:/Download/eclipse-juno/plugins/org.moflon.SDMLanguage_1.0.0.201305161247.jar", true);
      final URI sdmLanguagePlatformResourceURI = URI.createPlatformResourceURI("/SDMLanguage/", true);
      resourceSet.getURIConverter().getURIMap().put(sdmLanguagePlatformResourceURI, GenModelBuilder.createArchiveURI(sdmLanguageJarURI));
      final URI sdmLanguageGenModelURI = URI.createURI("model/SDMLanguage.genmodel").resolve(sdmLanguagePlatformResourceURI);

      // Load SDMLanguage ecore
      final SDMEnhancedEcoreResource sdmEcoreResource = (SDMEnhancedEcoreResource) resourceSet.getResource(sdmLanguageGenModelURI.trimFileExtension()
            .appendFileExtension("ecore"), true);
      ...
      final List<GenPackage> sdmGenPackages = ((GenModel) sdmLanguageGenModelResource.getContents().get(0)).getGenPackages();
      genModel.getUsedGenPackages().addAll(sdmGenPackages);

      // Initialize Democles URIs
      final URI democlesJarURI = URI.createPlatformResourceURI("/ControlFlow/lib/org.gervarro.democles.specification.emf.jar", true);
      ...
      // Load Democles ecore
      final SDMEnhancedEcoreResource democlesEcoreResource = (SDMEnhancedEcoreResource) resourceSet.getResource(democlesGenModelURI.trimFileExtension()
            .appendFileExtension("ecore"), true);
      ...
      final List<GenPackage> democlesGenPackages = ((GenModel) democlesLanguageGenModelResource.getContents().get(0)).getGenPackages();
      ...
      final SDMEnhancedEcoreResource cfEcoreResource = (SDMEnhancedEcoreResource) resourceSet.getResource(
            cfGenModelURI.trimFileExtension().appendFileExtension("ecore"), true);
      ...
      final List<GenPackage> cfGenPackages = ((GenModel) cfLanguageGenModelResource.getContents().get(0)).getGenPackages();
   ...
   {
//      final DemoclesMethodBodyHandler methodBodyHandler = new DemoclesMethodBodyHandler(set);
//      final EMoflonCodeGenerationApp genModelBuilder = new EMoflonCodeGenerationApp(set);
...
//
//      set.getURIConverter().getURIMap().put(URI.createPlatformResourceURI("/", true), URI.createFileURI(args[0] + "\\"));
//
...
//
//      final URI sequencerGenModelURI = URI.createPlatformResourceURI("/Sequencer/model/Sequencer.genmodel", true);
//      final GenModel genModel = genModelBuilder.buildGenModel(sequencerGenModelURI, false);
