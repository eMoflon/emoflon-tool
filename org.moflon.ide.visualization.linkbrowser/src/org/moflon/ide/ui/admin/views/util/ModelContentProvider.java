...
public class ModelContentProvider implements IGraphEntityRelationshipContentProvider
{
   ...
   private void addInputToContent(Object[] inputElements)
   {
      ...
      {
         if (object instanceof AttributeProxy)
         {
            AttributeProxy proxy = (AttributeProxy) object;
            // Exclude upperBound attribute proxies from content
            if (proxy.getName() != null && !proxy.getName().equals("upperBound"))
            {
               content.add(proxy);
            }
   ...
   public Object[] getRelationships(Object source, Object dest)
   {
      if (source instanceof EObject && dest instanceof EObject)
         ...
         throw new RuntimeException("Types not supported (src, dest): " + source.getClass().toString() + dest.getClass().toString());
   }
