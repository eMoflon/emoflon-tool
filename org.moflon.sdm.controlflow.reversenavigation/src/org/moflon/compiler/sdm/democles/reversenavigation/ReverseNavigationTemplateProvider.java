...
{
   @Override
   public Chain<TemplateController> getTemplateController(GeneratorOperation operation, Chain<TemplateController> tail)
   {
      ...
      if (type instanceof StructuralFeature<?>) {
         ...
         if (adornment.get(0) == Adornment.FREE && adornment.get(1) == Adornment.BOUND) {
            if (((EReference) sf.getLinkedElement()).getEOpposite() == null) {
               return new Chain<TemplateController>(new TemplateController("/reverseNavi/BackwardExtendToMany", operation), tail);
            }
         }
      }
