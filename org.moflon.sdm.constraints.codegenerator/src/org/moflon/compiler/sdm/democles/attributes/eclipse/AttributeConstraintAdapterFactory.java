...
public class AttributeConstraintAdapterFactory implements IAdapterFactory
{
   ...
   final Class adapterType)
   {
      if (adaptableObject instanceof MoflonCodeGenerator && AttributeConstraintCodeGeneratorConfig.class == adapterType)
      {
         ...
         final AttributeConstraintCodeGeneratorConfig defaultCodeGeneratorConfig = new AttributeConstraintCodeGeneratorConfig(process.getResourceSet(), project,
               process.getPreferencesStorage());
         return new DemoclesMethodBodyHandler(process.getResourceSet(), defaultCodeGeneratorConfig);
      } else if (adaptableObject instanceof DemoclesValidationProcess && AttributeConstraintCodeGeneratorConfig.class == adapterType)
      {
         final DemoclesValidationProcess process = (DemoclesValidationProcess) adaptableObject;
         IProject project = process.getEcoreFile().getProject();
         return new AttributeConstraintCodeGeneratorConfig(process.getResourceSet(), project, process.getPreferencesStorage());
      }
      return null;
   }
