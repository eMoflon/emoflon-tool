...
public abstract class OVTransformerRule implements IOperatorRule
{
   ...
   public String transforming(ObjectVariableDefinition ov, Variable variable, Map<String, Boolean> bindings, PatternBody patternBody, String oldSuffix)
   {
      ...
      if (isTransformable(ov, variable))
      {
         ...
         ov.getLinkVariablePatterns().stream().forEach(link -> {
            LinkVariableBuilder.getInstance().transformLinkVariable(link, ov, patternBody);
         });
         ov.getAttributeConstraints().stream().forEach(ac -> {
            ExpressionBuilder.getInstance().transformExpression(ov, ac, patternBody);
         });
         oldSuffix = suffix;
         return suffix;
      }
