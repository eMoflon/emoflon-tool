...
public abstract class AbstractStatementRule<S extends Statement>
{
   ...
   {
      StatementBuilder.setStatementRule(getStatementClass(), stmnt -> scope -> previosCFNode -> {
         castStatement(stmnt, scope, previosCFNode);
      }); // using currying here
   }
   ...
   protected void handlePattern(List<CalledPatternParameter> cpps, PatternDef patternDef, CFNode cfNode, Scope scope)
   {
      ...
      PatternBuilder patternBuilder = PatternBuilder.getInstance();
      patternBuilder.createPattern(patternDef, bindings, env, nameGenerator);
      ...
      for (int invocationIndex = invocations.size() - 1; invocationIndex >= 0; invocationIndex--)
      {
         ...
         cfVariables.stream().filter(cfVar -> patternBuilder.isConstructorPattern(cfNode, cfVar, invocation, methodParameters, patternName)).forEach(cfVar -> cfVar.setConstructor(invocation));
