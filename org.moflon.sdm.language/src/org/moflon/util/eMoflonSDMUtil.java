...
{
   /**
    * Convenience method to retrieve the SDM diagram (as Activity from SDMLanguage) of an operation, whose
    * implementation was specified by an SDM.
    * 
    * @param op
   ...
   /**
    * Used to delete an Activity (root of an SDM) if the operation is a MoflonOperation
    *
    * @param op
   ...
   public static String extractMethodCall(final MethodCallExpression methodCall, final String projectName)
   {
      ...
      {
         Optional<ParameterBinding> bindingOptional = methodCall.getOwnedParameterBindings().stream().filter(pb -> pb.getParameter().equals(param)).findAny();
   ...
   private static String fillArgumentListWithBinding(final String projectName, String argumentList, ParameterBinding binding)
   {
      ...
      } else if (argumentExp instanceof AttributeValueExpression)
      {
         ...
         throw new UnsupportedOperationException("Unsupported value of parameter binding (can only be textual or an object variable): " + argumentExp);
      return argumentList;
   }
   ...
   public static String extractValueFromLiteralExpression(final LiteralExpression literalExpression)
   {
      ...
      {
         // All other EDataTypes or null, are just copied directly into the generated code
   ...
   public static String extractAttributeValueExpression(final AttributeValueExpression attributeValueExpression, final String projectName)
   {
   ...
   {
      return attributeAssignment.getAttribute().getName() + " := " + eMoflonSDMUtil.extractExpression(attributeAssignment.getValueExpression(), "");
   }
