...
public class EnterpriseArchitectHelper {
	...
	public static void delegateToEnterpriseArchitect(final IProject project, final IProgressMonitor monitor)
			throws IOException, InterruptedException {
		delegateToEnterpriseArchitect(project, monitor,
				generateExportCommand(WorkspaceHelper.getEapFileFromMetamodelProject(project)), "Exporting");
	}

	private static String generateExportCommand(IFile eapFile) {
		URL pathToExe = WorkspaceHelper.getPathRelToPlugIn(COMMAND_LINE_EA_EXECUTABLE,
				WorkspaceHelper.getPluginId(EnterpriseArchitectHelper.class));
		return "\"" + new File(pathToExe.getPath()).getAbsolutePath() + "\" " + EXPORT_OPTION + EAP_EXTENSIONS + "\""
				+ eapFile.getLocation().toOSString() + "\"";

	}

	private static void delegateToEnterpriseArchitect(final IProject project, final IProgressMonitor monitor,
			final String command, final String importExport) throws IOException, InterruptedException {
		try {
	...
	private static String generateImportCommand(IFile eapFile, IFile xmiFile) {
		URL pathToExe = WorkspaceHelper.getPathRelToPlugIn(COMMAND_LINE_EA_EXECUTABLE,
				WorkspaceHelper.getPluginId(EnterpriseArchitectHelper.class));
		return "\"" + new File(pathToExe.getPath()).getAbsolutePath() + "\" " + IMPORT_OPTION + XMI_EXTENSIONS + "\""
				+ xmiFile.getLocation().toOSString() + "\" " + EAP_EXTENSIONS + "\""
				+ eapFile.getLocation().toOSString() + "\"";
	}
