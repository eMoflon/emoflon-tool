   ...
   public void extractInjection(final IFile javaFile, final boolean runsInteractive)
   {
      try
      {
         ...
         } else
         {
            ...
            InjectionFileBuilder.createInjectionFile(javaFile, resultStatus);
            if (!resultStatus.matches(IStatus.ERROR))
            {
               ...
               {
                  logger.debug("Not creating injection file for  " + javaFile + " because no model code were found.");
               }
            }
            else {
               logger.error("Unable to create injection code for " + javaFile + " due to " + resultStatus);
            }
