 ...
 * 
 * With the name of the given correspondence package, for example "BlockCodeAdapter", test data is located: input models
 * (/resources/tggLanguageTestData/in/BlockCodeAdapter) and expected models
 * (/resources/tggLanguageTestData/in/BlockCodeAdapter).
 * 
 * The created model is compared to the expected model and for each test, the output model, the translation protocol as
 * well as the correspondence model are stored to /resources/tggLanguageTestData/out/BlockCodeAdapter.
 * 
 */
public abstract class TransformationTest
{
   ...
   /**
    * Compares the created model with the expected model for the given transformation direction.
    *
   ...
   protected ApplicationTypes determineDirection(final String testCaseName)
   {
      ...
      {
         throw new RuntimeException("Test Case name \"" + testCaseName + "\" is invalid, it does not contain \"FWD\" or \"BWD\"!");
      }
   }
   ...
   protected EObject loadExpected(final String testCaseName, final ApplicationTypes direction)
   {
      try
      {
         Resource resource = helper.getResourceSet().getResource(eMoflonEMFUtil.createFileURI(getExpectedPath() + testCaseName + ".xmi", true), true);
