...
public final class EnterpriseArchitectValidationHelper implements ErrorReporter {
	...
	public final void sendResultToEA(final ValidationReport validationReport, final IPath metamodelProjectPath,
			final int errorCounterOffset) {
	...
	public final void sendErrorMessageToEA(final ErrorMessage errorMsg, final IPath metamodelProjectPath,
			final int errorCnt) {
		List<String> messages = new ArrayList<String>();
		messages.add("SETUP_VALIDATION_SESSION");
	...
	public final String getActivityEdgeString(final ActivityEdge activityEdge) {
		...
		builder.append(EAInterfaceUriHelper.DELIM);
		builder.append(EAInterfaceUriHelper.getActivityEdgeString(activityEdge.getSource().getName(),
				activityEdge.getTarget().getName()));
		return builder.toString();
	}
	...
	public void report(final IStatus status) {
		...
		if (isEARunning()) {
			...
			try {
				...
				logger.error(
						"Unable to communicate with EA: " + e.getMessage() + ", Please ensure that EA is running!");
			}
		}
	}
