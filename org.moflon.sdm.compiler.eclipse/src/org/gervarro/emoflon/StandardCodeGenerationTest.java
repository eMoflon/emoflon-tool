...
public class StandardCodeGenerationTest
{
   ...
   public static final void main(final String[] args)
   {
      ...
      // Accessing the generated version of ECore.ecore: "http://www.eclipse.org/emf/2002/Ecore" -> mapping to genmodel?
      ...
      resourceSet.getURIConverter().getURIMap()
            .put(URI.createPlatformPluginURI("/org.eclipse.emf.ecore/", true), URI.createURI("archive:" + ecoreJarLocation + JAR_SEPARATOR + "/", true));
      resourceSet.getURIConverter().getURIMap().put(URI.createPlatformResourceURI("/", true), URI.createFileURI(workspaceRootAsFile + "\\"));

      // Only makes sense in Eclipse mode:
      // resourceSet.getURIConverter().getURIMap().putAll(EcorePlugin.computePlatformURIMap());

      // (1) Load ECore file (platform:/plugin/projectName/model/file.ecore)
      URI ecoreModelLocation = URI.createPlatformResourceURI("/org.gervarro.democles.emoflon/model/Moflon.ecore", true);
      ...
      interestingRootEPackages.addAll(EcoreUtil.<EPackage> getObjectsByType(ecoreModelResource.getContents(), EcorePackage.Literals.EPACKAGE));

      List<EPackage> referencedRootEPackages = new ConverterUtil.EPackageList();
      for (Resource resource : resourceSet.getResources())
      {
         referencedRootEPackages.addAll(EcoreUtil.<EPackage> getObjectsByType(resource.getContents(), EcorePackage.Literals.EPACKAGE));
      }
      ...
      // (3) Produce GenModel contents from scratch (GenPackages and referred GenPackages)
