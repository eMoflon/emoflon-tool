...
public class MoflonGenModelBuilder extends GenModelBuilder {
	...
	public MoflonGenModelBuilder(final ResourceSet resourceSet, final List<Resource> resources, 
			final IFile ecoreFile, final String basePackage, final String modelDirectory,
	...
	public List<Dependency> getGenModelResourceDependencies() {
		...
		for (final Resource resource : resources) {
			...
			if (!isAdditionalDependency(uri) && !uri.equals(ecoreURI)) {
				final int kind = CodeGeneratorPlugin.getDependencyType(uri);
	...
	private final URI lookupExtensionRegistry(final URI uri) {
		...
		final IExtensionPoint extensionPoint =
				Platform.getExtensionRegistry().getExtensionPoint("org.eclipse.emf.ecore.generated_package");
