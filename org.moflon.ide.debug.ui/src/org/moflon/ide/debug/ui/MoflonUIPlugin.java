 ...
 */
public class MoflonUIPlugin extends EMoflonUIPlugin implements ILaunchesListener2 {
	...
	private final static String PATH_OBJECT = ICONS_PATH + "obj16/"; // Model //$NON-NLS-1$
																		// object
																		// icons
	...
	/**
	 * Active adapter sets. They are accessed using the DSF session ID which
	 * owns the debug services.
	 */
	...
	// map (used as a set) is maintained in
	// order
	// * to avoid re-creating an adapter set after the launch was removed from
	// the launch manager, but while the launch
	// is
	// * still being held by other classes which may request its adapters. A
	// weak map is used to avoid leaking memory
	// once
	...
	// * </p>
	// */
	// private Map<ILaunch, Object> fDisposedSessionAdapterSets = new
	// WeakHashMap<ILaunch, Object>();
	...
	private void declareImage(String key, String path) {
		// URL url = BundleUtility.find("org.eclipse.cdt.examples.dsf.pda.ui",
		// path);
