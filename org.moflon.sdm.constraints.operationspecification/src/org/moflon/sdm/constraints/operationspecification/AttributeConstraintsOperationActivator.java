...
public class AttributeConstraintsOperationActivator extends Plugin
{
    ...
    *  
    *  For instance, the {@link OperationSpecification} for the constraint 'max' with adornment FBB could look like
    *  <code>$a$ = Math.max($b$,$c$)</code>. Similarly, with adornment BBB <code>$a$ == Math.max($b$,$c$)</code>.
    *  (We assume that the constraint parameters are called a, b, c,... (as by default).
    */
