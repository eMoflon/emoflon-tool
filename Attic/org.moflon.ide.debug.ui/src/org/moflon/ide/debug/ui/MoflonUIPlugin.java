 ...
 */
public class MoflonUIPlugin extends AbstractUIPlugin implements ILaunchesListener2
{

   public static String PLUGIN_ID = "org.moflon.ide.debug.ui";

   // The shared instance.
   private static MoflonUIPlugin plugin;
   ...
   /**
    * Active adapter sets. They are accessed using the DSF session ID which owns the debug services.
    */
   // private Map<String, SessionAdapterSet> fSessionAdapterSets =
   // Collections.synchronizedMap(new HashMap<String, SessionAdapterSet>());

   // /**
   // * Map of launches for which adapter sets have already been disposed. This map (used as a set) is maintained in
   // order
   // * to avoid re-creating an adapter set after the launch was removed from the launch manager, but while the launch
   // is
   // * still being held by other classes which may request its adapters. A weak map is used to avoid leaking memory
   // once
   // * the launches are no longer referenced.
   // * <p>
   // * Access to this map is synchronized using the fSessionAdapterSets instance.
   // * </p>
   // */
    ...
    */
   public static MoflonUIPlugin getDefault()
   {
      return plugin;
   }
    ...
    * @param path
    *           the path of the image file relative to the base of the workbench plug-ins install directory
