...
public class ExpressionStatement extends StatementNode
{
    ...
    * @param expression
    */
   public ExpressionStatement(FParsedElement parsedElement, TextNode expression)
   {
      super(parsedElement);
    ...
    * @return
    */
   public boolean setExpression(final TextNode value)
   {
      boolean changed = false;

      if(this.expression != value)
      {
         final TextNode oldValue = this.expression;
         this.expression = value;

         // sync children link
         this.removeFromChildren(oldValue);
         this.addToChildren(value);
         if(this.sizeOfChildren() != 1)
         {
            throw new IllegalStateException("ExpressionStatement has invalid number of children: "
                  + this.sizeOfChildren());
         }
