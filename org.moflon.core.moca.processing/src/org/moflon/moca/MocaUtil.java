...
public class MocaUtil
{
    ...
    * @param lexer
    * @param leftMargin
    *           amount of characters that will be removed from the beginning of the token
    * @param rightMargin
    *           amount of characters that will be removed from the end of the token
    */
   ...
   public static CommonTree mocaTreeToCommonTree(Text element, String[] tokenNames, boolean sort)
   {
      ...
      /*
       * Text elements are transformed to a CommonTree ^(TEXT name) The root element of a MocaTree must be a node.
       */
      else
      {
         if (element.getParentNode() == null)
         {
            throw new IllegalArgumentException("The root element of the given MocaTree is a Text element. Only Node elements can be the root of a MocaTree!");
         } else
         {
    ...
    * @param tokenNames
    */
   private static void transformTreeElements(CommonTree result, List<? extends TreeElement> elements, String[] tokenNames, boolean sort)
   {
   ...
   private static CommonTree transformTreeElement(String[] tokenNames, TreeElement treeElement, boolean sort)
   {
      ...
      {
         throw new IllegalArgumentException("The given tree element must be Attribute, Node or Text, given: " + treeElement);
      }
      return childTree;
   }

   /**
    * Transforms CommonTree to MocaTree. The node label is set to the text entry of the corresponding CommonTree.
    * 
    * @param tree
    * @param tokenNames
    * @return
   ...
   private static Text createTextFromCommonTree(CommonTree tree)
   {
      ...
      {
         throw new IllegalArgumentException("The TEXT tree " + tree.toStringTree() + " is invalid. Valid text trees have the form ^(TEXT content)");
      }
